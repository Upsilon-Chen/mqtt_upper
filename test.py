...
class UpperComputer(QtWidgets.QWidget):
    ...
    def initUI(self):
        ...
        self.link_tab = QtWidgets.QWidget()
        ...
        # 添加选项卡 
        self.tabs.addTab(self.link_tab, "连接")
        self.tabs.addTab(self.control_tab, "控制")
        self.tabs.addTab(self.log_tab, "日志")
        self.tabs.addTab(self.save_tab, "保存的数据")

        # 连接选项卡布局
        link_layout = QtWidgets.QVBoxLayout(self.link_tab)

        # 连接按钮
        self.broker_edit = QtWidgets.QLineEdit()
        self.broker_edit.setPlaceholderText('请输入broker地址')
        ...
        link_layout.addWidget(self.broker_edit)
        link_layout.addWidget(self.port_edit)
